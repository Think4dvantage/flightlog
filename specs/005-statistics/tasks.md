...
- [ ] T001 [P] Re-confirm Chart.js is still `v4.5.1` (`gh api repos/chartjs/Chart.js/releases/latest`)
      if implementation starts a meaningful time after this plan

## Phase 2 — Foundation

- [ ] T002 Create `src/flightlog/models/stats.py` — every Pydantic response schema from `data-model.md`
...
- [ ] T003 [P] Build a small hand-crafted fixture set in `tests/backend/test_stats.py`'s own fixtures
...
- [ ] T004 [US1] Implement `totals()`, `time_breakdown()`, `distribution()`, `personal_bests()` in
...
- [ ] T005 [US1] [P] `tests/backend/test_stats.py` — totals/breakdown/distribution/personal-bests
      against the T003 fixtures, including the tie case
- [ ] T006 [US1] Add `GET /api/stats/totals`, `/time-breakdown`, `/distribution`, `/personal-bests` to
...
- [ ] T007 [US1] Implement the shared `year_matrix()` helper in `core/stats.py` and its five callers
      (site/region/glider/harness/category), per `research.md`'s shared-query decision
- [ ] T008 [US1] Implement `launch_technique_split()` in `core/stats.py`
- [ ] T009 [US1] [P] `tests/backend/test_stats.py` — matrix "not recorded" bucket case, launch-technique
      split and percentage
- [ ] T010 [US1] Add `GET /api/stats/matrix/{dimension}` and `/launch-technique` to `stats.py`
...
- [ ] T011 [US2] Implement `igc_rollup()` in `core/stats.py` — `SUM(igc_segments.alt_change_m) WHERE
      kind = 'thermal'`, joined across the owner's tracks
- [ ] T012 [US3] Implement the buddy year matrix (reusing `year_matrix()`'s shape, extra join per
      `plan.md`'s risk note) in `core/stats.py`
- [ ] T013 [US3] Implement `current_streak()`, `ytd_pace()`, `cumulative_progression()` in `core/stats.py`
- [ ] T014 [US2] [US3] [P] `tests/backend/test_stats.py` — zero-track empty state, sparse-then-populated
      buddy matrix, streak/pace against a small known fixture calendar
- [ ] T015 [US2] [US3] Add `GET /api/stats/igc-rollup`, `/matrix/buddy` (or fold into T010's
      `{dimension}` route if `buddy` fits the same shape cleanly), `/progression` to `stats.py`

## Phase 6 — Frontend

- [ ] T016 Create `static/stats.html`/`static/stats.js` — totals section, time-breakdown chart, personal
...
- [ ] T017 Add `GET /stats` route to `src/flightlog/api/routers/pages.py`; nav entry (`nav.stats`,
      already referenced in `bootstrap.js`'s `NAV_LINKS` since `v0.3` but never linked to a real page) in
      `static/bootstrap.js`
- [ ] T018 [P] Add `stats.*` i18n keys, including explicit empty-state copy for the IGC rollup and buddy
      matrix (per `plan.md`'s risk note — must read as "not built up yet," not "broken")

## Final Phase — Polish

- [ ] T019 Live-boot verification against the real 600-flight data: spot-check totals/averages by hand;
...
- [ ] T020 Confirm every NFR-003 zero-state renders cleanly on a scratch account with no flights
- [ ] T021 `ruff check`/`ruff format --check`/full `pytest` clean; bump `pyproject.toml`'s version
- [ ] T022 `sync.md` — update `architecture.md` (API Contracts table, note the third confirmed workbook
