...
- [ ] T001 Re-verify Leaflet and Chart.js are still the latest stable releases (`gh api
      ...
      planning time
- [ ] T002 [P] Vendor the missing Leaflet 1.9.4 marker/layer images (`marker-icon.png`,
...
- [ ] T003 Create `static/refdata.js` — fetch-once, in-memory-cached helper for `GET /api/sites`,
...
- [ ] T004 [US1] Add `GET /flights` route serving `static/flights.html` in
      `src/flightlog/api/routers/pages.py`
- [ ] T005 [US1] Create `static/flights.html` — page shell, search box, filter row, sortable table header,
      pagination controls (nav link already present in `bootstrap.js`'s `NAV_LINKS`)
- [ ] T006 [US1] Create `static/flights.js` — `fetchAuth('/api/flights')` once, join via `refdata.js`,
...
- [ ] T007 [US1] Add `flights.*` keys (search placeholder, column headers, filter labels, empty state,
...
- [ ] T008 [US2] Extend `static/flights.html` with the add/edit drawer markup — every `FlightCreate`/
      `FlightUpdate` field, a buddy multi-select, a delete-confirmation control
- [ ] T009 [US2] Extend `static/flights.js` — open/close the drawer, populate dropdowns from
      ...
      view
- [ ] T010 [US2] Add `flights.drawer.*` and `flights.delete_confirm.*` keys to `static/i18n/en.json`
...
- [ ] T011 [US3] Add `GET /flights/{flight_id}` route serving `static/flight-detail.html` in
      `src/flightlog/api/routers/pages.py`
- [ ] T012 [US3] Create `static/flight-detail.html`
- [ ] T013 [US3] Create `static/flight-detail.js` — read the flight id from the URL, `GET
...
- [ ] T014 [US3] Add `flight_detail.*` keys to `static/i18n/en.json`
- [ ] T015 [US3] Link each row in `static/flights.js`'s list to its `/flights/{id}` detail page
...
- [ ] T016 [US4] Modify `src/flightlog/api/routers/sites.py`: `POST`/`PUT` set `coord_source = "manual"`
      server-side whenever the request includes a non-null `lat` and/or `lon` (`contracts/
      sites-coord-source.md`)
- [ ] T017 [US4] [P] Extend `tests/backend/test_sites.py`: `coord_source` becomes `"manual"` on create
      with `lat`/`lon`, becomes `"manual"` on update with `lat`/`lon`, stays untouched when neither is
      present
- [ ] T018 [US4] Add `GET /sites` route serving `static/sites.html` in
      `src/flightlog/api/routers/pages.py`
- [ ] T019 [US4] Create `static/sites.html` — site list table plus a Leaflet map container (nav link
      already present in `bootstrap.js`'s `NAV_LINKS`)
- [ ] T020 [US4] Create `static/sites.js` — `GET /api/sites`, render the list, initialize Leaflet against
...
- [ ] T021 [US4] Add `sites.*` keys to `static/i18n/en.json`
...
- [ ] T022 [US5] Add `GET /equipment` route serving `static/equipment.html` in
      `src/flightlog/api/routers/pages.py`
- [ ] T023 [US5] Create `static/equipment.html` (nav link already present in `bootstrap.js`'s
      `NAV_LINKS`)
- [ ] T024 [US5] Create `static/equipment.js` — CRUD against `GET/POST/PUT /api/gliders` and
      `/api/harnesses`, `POST .../{id}/retire`, retired-item styling
- [ ] T025 [US5] Add `equipment.*` keys to `static/i18n/en.json`
- [ ] T026 [US5] Update `static/flights.js`'s drawer (from Phase 4) so its glider/harness dropdowns
...
- [ ] T027 [US6] Run a dry-run of `run_import()` against `olddata/Flugbuch.xlsx` and use its
...
- [ ] T028 [US6] Create `src/flightlog/models/import_report.py` (`data-model.md`'s response schema)
- [ ] T029 [US6] Create `src/flightlog/api/routers/import_report.py` — `GET /api/import-report`
      (`contracts/import-report.yaml`)
- [ ] T030 [US6] [P] Register the `import_report` router in `src/flightlog/api/main.py`
- [ ] T031 [US6] [P] Create `tests/backend/test_import_report.py` — 200 with the documented shape,
      requires auth, values match the frozen constant from T027
- [ ] T032 [US6] Add `GET /import` route serving `static/import.html` in
      `src/flightlog/api/routers/pages.py`
- [ ] T033 [US6] Add a `{ page: 'import', href: '/import', key: 'nav.import' }` entry to `NAV_LINKS` in
      `static/bootstrap.js`
- [ ] T034 [US6] Create `static/import.html`
- [ ] T035 [US6] Create `static/import.js` — `GET /api/import-report`, render each finding group, an
      explicit "this is a historical snapshot, not a live view" note per the spec's Edge Cases
- [ ] T036 [US6] Add `nav.import` and `import.*` keys to `static/i18n/en.json`
