...
|---|---|---|
| `users` | **shipped v0.1**, `public_profile_enabled` added v0.9, `public_stats_enabled` added v0.9.5 | `id`, `email` (unique), `display_name`, `hashed_password`, `role`, `is_active`, `locale`, `timezone`, `units`, `seeded_at`, `public_profile_enabled`, `public_stats_enabled`, `last_login_at`, `created_at`, `updated_at` — `seeded_at` was reserved-but-unused plumbing from v0.2 through v0.8; v0.9's `core/user_seed.py` is its first real consumer. `public_stats_enabled` is deliberately independent of `public_profile_enabled` — sharing your flight list and sharing your lifetime aggregate numbers are different disclosure decisions |
...
| `flight_links` | **shipped v0.8**, pilot write path added v0.9.6 | `id`, `flight_id`, `kind`, `external_id`, `url`, `label`, `created_at`, `updated_at` — `UniqueConstraint(flight_id, kind, external_id)`; a `PUT` to that triple replaces, never duplicates. No `owner_id` — reached through the parent `flights` row, same reasoning as `igc_segments`. VidFactory pushes here via `PUT /api/integration/v1/flights/{id}/links/{kind}/{external_id}`; the pilot's own `GET /api/flights/{id}` (and the list endpoint, one small per-flight query each — same precedent as `buddy_ids`) surfaces them back as `FlightOut.links` with no action needed (FR-009). **v0.9.6**: `POST`/`DELETE /api/flights/{id}/links[/{link_id}]` give the pilot their own first write path onto this table — `kind` restricted to `video`\|`xcontest` there (the column itself stays open-ended for VidFactory), `external_id` a fresh `new_uuid()` since there's no natural id to derive from a pasted link. A manually-pasted YouTube link reuses VidFactory's own `kind="video"` rather than a new `"youtube"` kind, so both sources render in one unified list |
